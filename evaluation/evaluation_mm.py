...
@torch.no_grad()
def evaluate_ret(model, tasks, val_loader, global_step, dset_name):
    val_log = {}
    ids = []
    ids_txt = []
    input_ids = []
    ...
    for i, batch in enumerate(val_loader):
        ...
        if 'feat_a' in evaluation_dict:
            feat_a.append(evaluation_dict['feat_a'])
            feat_name_list.append('feat_a')
        if 'feat_v' in evaluation_dict:
            feat_v.append(evaluation_dict['feat_v'])
            feat_name_list.append('feat_v')
        if 'feat_s' in evaluation_dict:
            feat_s.append(evaluation_dict['feat_s'])
            feat_name_list.append('feat_s')
    ...
    if 'feat_a' in feat_name_list:
        feat_a = torch.cat(feat_a, dim = 0)
        feat_a = ddp_allgather(feat_a)
        feat_list.append(feat_a)
    if 'feat_v' in feat_name_list:
        feat_v = torch.cat(feat_v, dim = 0)
        feat_v = ddp_allgather(feat_v)
        feat_list.append(feat_v)
    if 'feat_s' in feat_name_list:
        feat_s = torch.cat(feat_s, dim = 0)
        feat_s = ddp_allgather(feat_s)
        feat_list.append(feat_s)
        
    
        
    if dist.get_rank() == 0:
        for i, feat in enumerate(feat_list):
            print(f"feat_{i} shape: {feat.shape}")
    U, S = eigenvalue_computation_pmcl(feat_list)
    
    val_log[f"{dset_name}_eigenvalue_max"] = {"value": torch.mean(S[:,0]).item()}
    
    val_log[f"{dset_name}_eigenvector_uniformity"] = {"value": torch.mean(U[:,:,0] @ U[:,:,0].T).item()}
    ...
    for task in subtasks:
        ...
        val_log[f'{dset_name}_ret_itc_{task}'] = log
        
        del store_dict[f'feat_cond_{task}']
        torch.cuda.empty_cache()


    #### compute itm_score
    for task in subtasks:
        store_dict[f'condition_feats_{task}'] = torch.cat(store_dict[f'condition_feats_{task}'],dim=0).to(next(model.parameters()).device)
        if dist.get_rank() == 0:
            print(f"feat_cond_{task} shape: {store_dict[f'condition_feats_{task}'].shape}")
        # no ddp_allgather here
        ...
        val_log[f'{dset_name}_ret_itm_{task}'] = log
        del store_dict[f'condition_feats_{task}']
        torch.cuda.empty_cache()
...
def refine_score_matrix(condition_feats, input_ids, attention_mask, score_matrix_t_cond, model, itm_rerank_num, direction='forward'):
    ...
    for i in range(cur_length):
        ...
        for k in range(times):
            ...
            slice_scores = model.compute_slice_scores(slice_condition_feats.to(next(model.parameters()).device), slice_input_ids, slice_attention_mask) 
